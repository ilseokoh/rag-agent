...
from company_qna.sub_agents.hr_qna import prompt
...
tools = []
rag_corpus = os.environ.get("HR_RAG_CORPUS")
...
hr_agent = Agent(
    name="hr_qna",
    model=model_id,
    instruction=prompt.HR_INSTRUCTION,
    tools=tools,
    planner=planner,
    generate_content_config=generate_content_config,
    include_contents='default', # 이전 대화 기록을 사용 'none': 미사용
    output_key='hr_rag_result' # 최종 응답 텍스트 내용이 Session state에 자동 저장됨
)
